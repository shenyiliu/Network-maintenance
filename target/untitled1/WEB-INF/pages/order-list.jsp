						...
						<div class="table-box">
							...
							<div class="pull-left">
								<div class="form-group form-inline">
									<div class="btn-group">
										<security:authorize access="hasRole('ELEMENT_order:add')">
											<button type="button" class="btn btn-default" title="新建" onclick="window.location='${pageContext.request.contextPath}/url/order-add'">
												<i class="fa fa-file-o"></i> 新建
											</button>
										</security:authorize>
										<security:authorize access="hasRole('ELEMENT_order:delete')">
...
<%--							数据列表--%>
	...
	<script>
		...
		function getOrderPageHelper(pageNum,pageSize){
			...
			if (roles=="系统管理员"){
				...
				urlItem=$webName + "/order/selectOrderUserID";
			}
		...
		function addTable(pageInfo){
			...
			$.each(pageInfo.list,function(index,order){
				...
				"<td>"+order.phone +"</td>"+
				"<td>"+order.address +"</td>"+
				"<td class='text-center'>"+(order.orderStatue == 0? "未接" : (order.orderStatue == 1? "已接" : "已完成") )+"</td>"+
				"<td class='text-center'>"+
				"<security:authorize access='hasRole(\'ELEMENT_order:delete\')'>"+
						"<button type='button' class='deleteOrder btn bg-olive btn-xs'>删除</button>"+
				"</security:authorize>"+
				"<security:authorize access='hasRole(\'ELEMENT_order:update\')'>"+
		...
		$(document).on("click",".detailOrder",function(){
			...
			window.location.href = $webName + "/order/getOrderDetailById?id=" + id;
		});
		...
		$(document).on("click",".deleteOrder",function(){
			var id = $(this).parents("tr").children("td").eq(1).text();
			if(confirm("确认删除"+id+"订单吗")){
				$.ajax({
					url:$webName + "/order/deleteOrderById",
					type:"post",
					data:"id="+id,
		...
		$(document).on("click","#deleteOrders",function(){
			...
			if(confirm("确认删除订单吗")){
				$.ajax({
					url:$webName + "/order/deleteOrders",
					type:"post",
					data:"ids="+ids,
					success:function(flag){
		...
		$(document).on("click",".updateOrder",function(){
			var id = $(this).parents("tr").children("td").eq(1).text();
			window.location.href ="${pageContext.request.contextPath}/order/selectOrderUserByID?id=" + id;
		});
